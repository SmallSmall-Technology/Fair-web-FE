...
const deliverySlice = createSlice({
  ...
  reducers: {
    ...
    setDeleteAddress: (state, action) => {
      ...
      // Clear only if deleted address is currently selected
      if (state.current_address?.id === deletedId) {
        state.current_address = null;
      }
      if (state.latest_address?.id === deletedId) {
        state.latest_address = null;
      }
      if (state.delivery_address?.id === deletedId) {
        state.delivery_address = null;
      }
    },
    clearDeliveryAddresses: (state) => {
