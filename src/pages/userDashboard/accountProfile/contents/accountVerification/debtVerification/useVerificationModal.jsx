...
export const DebtVerificationModal = ({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (formData) => verifyDebtProfile(formData),
    onSuccess: (res) => {
      dispatch(setDebt(res?.data));

      const isApproved = res?.data?.length > 1; // ✅ custom approval logic
      setVerified(isApproved);
      setFailed(!isApproved);
      closeModal();

      if (!isApproved) {
        toast.error('Debt verification failed');
      } else {
